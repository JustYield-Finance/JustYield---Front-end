...
export class BeefyAPI {
  ...
  constructor() {
    ...
    this.api = axios.create({
      ...
      //baseURL: process.env.REACT_APP_API_URL || 'https://website-api-f5aj.onrender.com',
      timeout: 30 * 1000,
    });
    this.data = axios.create({
      baseURL: process.env.REACT_APP_DATA_URL || 'https://dogecompounder.herokuapp.com',
      timeout: 30 * 1000,
    });
  }
  ...
  public async getPrices(): Promise<BeefyAPITokenPricesResponse> {
    if (featureFlag_simulateBeefyApiError('prices')) {
      throw new Error('Simulated justyield api error');
    }
